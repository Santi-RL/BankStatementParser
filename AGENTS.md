...
- Autorización permanente del usuario para este proyecto: cuando corresponda ejecutar `autoreview` según estas reglas, queda preaprobado enviar el diff local necesario al motor externo Codex/OpenAI y mantener habilitada la búsqueda web del helper. No volver a pedir autorización solo por esa transferencia de diff. Esta autorización no habilita ejecutar `autoreview` de forma automática fuera de los criterios anteriores ni reemplaza aprobaciones que la plataforma exija técnicamente.
- Configuración predeterminada de `autoreview`: usar el motor Codex/OpenAI con el modelo por defecto vigente del helper y nivel de razonamiento alto (`--thinking high`), salvo que el usuario especifique explícitamente otro motor, modelo o esfuerzo de razonamiento.
